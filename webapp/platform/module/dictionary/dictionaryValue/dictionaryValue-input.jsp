		...
		var validateOptions = {
				rules: {
					"model.value" : {
						...
						remote:{
							type:"POST",
							url:"${path}/sys/dictionaryValue/ajaxCheckValue",
							data:{
								dictId:"${model.dictionary.id}",
								value:function(){return $("#${pageId}value").val();},
								excludeValue:"${model.value}"
			                } 
						}
					}
				},
		...
		<uc:form id="${pageId}theForm" method="post">
			...
			<div class="formLayout01">
				...
				<div class="formItem">
				     ...
				     <div class="bdmain w6">
				     	<uc:textfield id="${pageId}value" name="model.value" value="${model.value}" required="true" cssClass="inputStyle"/>
				     </div>
				</div>
