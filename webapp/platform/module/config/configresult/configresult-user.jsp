...
<head>
	<script>
		Namespace.register('com.ue.platform.module.config.configresult.user');
		com.ue.platform.module.config.configresult.user = {
			pageId : "${pageId}",
			doSave : function() {
				var self = this;
				$("body").mask('正在保存数据……');
...
// 				return;
				$.ajax({
					type : "POST",
					url : '${path}/sys/configresult/saveUserConfigResult',
					processData : true,
					data : formData,
					async : false,
		...
		$(document).ready(function() {
			com.ue.platform.module.config.configresult.user.init();
		});
	</script>
</head>
<body>
	<uc:form id="${pageId}theForm" method="post">
			<div class="formLayout01 mw500">
				<c:if test="${not empty configResultListVO}">
					<c:forEach items="${configResultListVO.configs}" var="config">
							<div class="formItem">
								...
								<div class="bdmain detail_right w7">
									<c:if test="${config.modeRadio}">
										 <c:forEach items="${configResultListVO.configItemMap[config.id]}" var="item">
										 	<label><input type="radio" id="${item.id}" value="${item.value}" name="${config.id}Name" 
										 	<c:if test="${item.value==configResultListVO.configResultValueMap[config.id]}">checked="checked"</c:if>/>${item.name}</label>
										 </c:forEach>
									</c:if>
									<c:if test="${config.modeCheckbox}">
										<c:forEach items="${configResultListVO.configItemMap[config.id]}" var="item">
										 	<label><input type="checkbox" id="${item.id}" value="${item.value}" name="${config.id}Name" 
										 	<c:if test="${fn:contains(configResultListVO.configResultValueMap[config.id],item.value)}">checked="checked"</c:if>/>${item.name}</label>
										 </c:forEach>
									</c:if>
									<c:if test="${config.modeInput}">
										<uc:textfield name="${config.id}Name" value="${configResultListVO.configResultValueMap[config.id]}"></uc:textfield>
									</c:if>
								</div>
							</div>	
					</c:forEach>
			...
			<div class="formButton">
				<div id="${pageId}btn_toEdit">
					<a href="javascript:;" onclick="com.ue.platform.module.config.configresult.user.toEdit();">编辑</a>
				</div>
				<div id="${pageId}btn_doSave" style="display: none">
					<a href="javascript:;" onclick="com.ue.platform.module.config.configresult.user.doSave();">保存</a>
					<a href="javascript:;" onclick="com.ue.platform.module.config.configresult.user.toView();">取消</a>
				</div>
			</div>
	</uc:form>
</body>
