			 ...
			 $.ajax({
			 	type: "POST",
			  	...
			  	success: function(data){
			  		$.successTips();
			  		com.ue.platform.module.attachment.attachmentConfig.list.refresh();
		...
		var validateOptions = {
			rules: {
				"model.code" : {
					...
					remote:{
						type:"POST",
						url:"${path}/sys/attachmentConfig/ajaxCheckCode",
						data:{
	...
	<div class="formLayout01">
		<div class="formItem">
		     ...
		     <div class="bdmain">
	     		...
	     		<uc:textfield id="${pageId}code" name="model.code" value="${model.code}" maxlength="25" cssClass="inputStyle" required="true" readonly="true" />
	     		</c:if>
		     	<c:if test="${model.code ne 'default' }">
	     		<uc:textfield id="${pageId}code" name="model.code" value="${model.code}" maxlength="25" required="true" cssClass="inputStyle" />
	     		</c:if>
	     		编码确定后请勿修改
		     </div>
		</div>
