...
//		}
		...
		if (!isNaN(height)) {
			if ($.boxModel == true) {
				...
				panels.css('height', height - tabHeight-5);
			}
		}
...
//		}
	...
	function selectTab(container, title){
		if (title) {
			...
			selectTabWithDefault(container);
		}
	}
	
	function selectTabWithTitle(container,title){
		if (title) {
			var elem = $('>div.tabs-header li:has(a span:contains("' + title + '"))', container)[0];
			if (elem) {
				$(elem).trigger('click');
	...
	function selectTabWithUid(container,uid){
		if (uid) {
			var elem = $('>div.tabs-header li[uid="' + uid + '"]', container)[0];
			if (elem) {
				$(elem).trigger('click');
	...
	function selectTabWithDefault(container){
	...
	function init(options){
		...
		var rs = this.each(function(){
			...
			function onClickTab() {
				...
				opts.onSelect.call(panel, tabAttr.title,tabAttr.uid);
			}
	...
	$.hz.tabs={
		init:function(tabId,options){
			return init.apply($("#"+tabId) ,[options]);
		},
		...
		close:function(tabId,uid){
			$("#"+tabId).each(function(){
				closeTab(this, uid, false);
			});
		},
		selectWithTitle:function(tabId,title){
			$("#"+tabId).each(function(){
				selectTabWithTitle(this, title);
			});
		},
		selectWithUid:function(tabId,uid){
			$("#"+tabId).each(function(){
				selectTabWithUid(this, uid);
			});
		}
	};
